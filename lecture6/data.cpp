...
public:
    ...
    std::string VertragVerlaengernUm;
    std::string VertragKuendigungsfrist;
    std::string VPeMail;
};
...
int main() {
    ...
    while (std::getline(file, line)) {
        ...
        vertrag.VertragVerlaengernUm = tokens[14];
        vertrag.VertragKuendigungsfrist = tokens[15];
    ...
    for (const Kunde& kunde : kunden) {
        ...
        for (const std::map<std::string, Vertrag>& vertragsMap : kunde.Vertragsliste) {
            for (const auto& pair : vertragsMap) {
                ...
                std::cout << "Vertrag Verlängern um: " << vertrag.VertragVerlaengernUm << std::endl;
                std::cout << "Vertrag Kündigungsfrist: " << vertrag.VertragKuendigungsfrist << std::endl;
